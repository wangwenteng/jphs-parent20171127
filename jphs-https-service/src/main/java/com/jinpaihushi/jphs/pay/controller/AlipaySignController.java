...
//										double total_fee_ord = Double.parseDouble(payPrice);
	...
	@Transactional
	public byte[] alipayNotifyGoods(HttpSession hs, HttpServletRequest request, HttpServletResponse response) throws IOException{
				//获取支付宝POST过来反馈信息
				Map<String,String> params = new HashMap<String,String>();
				Map requestParams = request.getParameterMap();
				for (Iterator iter = requestParams.keySet().iterator(); iter.hasNext();) {
					...
					if(trade_status.equals("TRADE_FINISHED")){
						...
						try {
							...
							if(commodityOrders_list!= null && !"".equals(commodityOrders_list) && commodityOrders_list.size() > 0){
								// 记录日志-debug
								if (Util.debugLog.isDebugEnabled()) {
									Util.debugLog.debug("alipay.notify.json;根据支付宝返回no="+no+"平台内部订单号查询订单结果：orders="+JSONArray.fromObject(commodityOrders_list).toString());
								}
								String userId = "";
