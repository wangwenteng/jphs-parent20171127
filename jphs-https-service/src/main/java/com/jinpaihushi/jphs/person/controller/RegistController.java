		...
		try {
			...
			if (Util.debugLog.isDebugEnabled()) {
				Util.debugLog.debug("regist.user.json,phone=" + user.getPhone() + " verificattionCode="
						+ verificattionCode + " type=" + type +" device="+user.getDevice() +" password="+user.getPassword());
			}
...
//	http://192.168.7.66:8059/regist/updatePwd.json?phone=手机号&verificattionCode=验证码&password=密码&type=2
	...
	@ResponseBody
	public byte[] updatePwd(HttpSession hs, HttpServletRequest req, HttpServletResponse resp,String phone, String verificattionCode,String password,String type){
		try {
			// 记录日志-debug
			if (Util.debugLog.isDebugEnabled()) {
				Util.debugLog.debug("regist.updatePwd.json 请求参数-phone"+phone+" verificattionCode"+ verificattionCode+" password"+password);
			}
			// 查空
			if (StringUtils.isEmpty(phone) || StringUtils.isEmpty(password)|| StringUtils.isEmpty(type) || StringUtils.isEmpty(verificattionCode)) {
				return JSONUtil.toJSONResult(0, "参数不能为空", null);
			}
			// 获取该手机号最新一条短信
	...
	@ResponseBody
	public byte[] ifValidateCode(HttpSession hs, HttpServletRequest req, HttpServletResponse resp,String phone, String verificattionCode){
		try {
			// 记录日志-debug
			if (Util.debugLog.isDebugEnabled()) {
				Util.debugLog.debug("regist.ifValidateCode.json 请求参数-phone"+phone+" verificattionCode"+ verificattionCode);
			}
			// 查空
	...
	public byte[] distinctPhone(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, String phone,String type) {
		try {
			// 记录日志-debug
			if (Util.debugLog.isDebugEnabled()) {
